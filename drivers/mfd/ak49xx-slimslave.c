		   ...
		   unsigned int rx_num, unsigned int *rx_slot)
{
	...
	if (ret) {
		pr_err("%s: xxxa5 Failed to configure register address offset\n",
		       __func__);
		goto err;
	}
	...
	if (ak49xx->rx_chs) {
		...
		ret = ak49xx_alloc_slim_sh_ch(ak49xx, ak49xx_pgd_la,
						...
						SLIM_SINK);
		if (ret) {
			pr_err("%s: xxxa50 Failed to alloc %d rx slimbus channels\n",
		...
		pr_err("xxxa51 Not able to allocate memory for %d slimbus rx ports\n",
			ak49xx->num_rx_port);
	}
	...
	if (ak49xx->tx_chs) {
		...
		ret = ak49xx_alloc_slim_sh_ch(ak49xx, ak49xx_pgd_la,
						...
						SLIM_SRC);
		if (ret) {
			pr_err("%s: xxxa52 Failed to alloc %d tx slimbus channels\n",
		...
		pr_err("xxxa53 Not able to allocate memory for %d slimbus tx ports\n",
			ak49xx->num_tx_port);
	}
			    ...
			    u16 *grph)
{
	...
	list_for_each_entry(rx, ak49xx_ch_list, list) {
		...
		ch_h[ch_cnt] = rx->ch_h;
		ch_cnt++;
		pr_err("xxxa4 list ch->ch_h %d ch->sph %d\n", rx->ch_h, rx->sph);
	}
	pr_debug("%s: xxxa40 ch_cnt[%d] rate=%d\n", __func__, ch_cnt, rate);
	/* slim_define_ch api */
	...
	list_for_each_entry(rx, ak49xx_ch_list, list) {
		codec_port = rx->port;
		pr_err("%s: xxxa43 codec_port %d rx 0x%p, payload %d\n"
		...
		*/
		...
		if (ret < 0) {
			pr_err("%s: xxxa44 slim_connect_sink failed ret[%d]\n",
				__func__, ret);
	...
	ret = slim_control_ch(ak49xx->slim, *grph, SLIM_CH_ACTIVATE,
			      true);
	if (ret < 0) {
		pr_err("%s: xxxa45 slim_control_ch failed ret[%d]\n",
			__func__, ret);
		goto err_close_slim_sch;
	}
	return 0;
			    ...
			    u16 *grph)
{
	...
	ret = slim_define_ch(ak49xx->slim, &prop, ch_h, ch_cnt,
			     true, grph);
	if (ret < 0) {
		pr_err("%s: xxxa1 slim_define_ch failed ret[%d]\n",
		       __func__, ret);
		goto err;
	}

	pr_err("%s: xxxa10 ch_cnt[%d] rate[%d]\n", __func__, ch_cnt, rate);
	list_for_each_entry(tx, ak49xx_ch_list, list) {
		codec_port = tx->port;
		pr_err("%s: xxxa11 codec_port %d tx 0x%p, payload 0x%x\n",
		...
		*/
		...
		if (ret < 0) {
			pr_err("%s: xxxa12 slim_connect_src failed ret[%d]\n",
			       __func__, ret);
			goto err;
		}
	}
	...
	ret = slim_control_ch(ak49xx->slim, *grph, SLIM_CH_ACTIVATE,
			      true);
	if (ret < 0) {
		pr_err("%s: xxxa13 slim_control_ch failed ret[%d]\n",
			__func__, ret);
		goto err;
	}
	return 0;
			      ...
			      struct list_head *ak49xx_ch_list, u16 grph)
{
	...
	pr_err("%s xxxa5 ch_cht %d, sph[0] %d sph[1] %d\n", __func__, ch_cnt,
		sph[0], sph[1]);

	/* slim_control_ch (REMOVE) */
	pr_err("%s xxxa50 before slim_control_ch grph %d\n", __func__, grph);
	ret = slim_control_ch(ak49xx->slim, grph, SLIM_CH_REMOVE, true);
	if (ret < 0) {
		pr_err("%s: xxxa51 slim_control_ch failed ret[%d]\n", __func__, ret);
		goto err;
	}
err:
	return ret;
			      ...
			      u16 grph)
{
	...
	pr_err("%s xxxa20 ch_cht %d, sph[0] %d sph[1] %d\n",
		__func__, ch_cnt, sph[0], sph[1]);
	/* slim_control_ch (REMOVE) */
	ret = slim_control_ch(ak49xx->slim, grph, SLIM_CH_REMOVE, true);
	if (ret < 0) {
		pr_err("%s: xxxa21 slim_control_ch failed ret[%d]\n",
			__func__, ret);
		goto err;
	}
err:
			    ...
			    struct list_head *ak49xx_ch_list, u16 grph)
{
	...
	if (ret < 0) {
		pr_err("%s: xxxa3 slim_disconnect_ports failed ret[%d]\n",
			__func__, ret);
	}

	pr_err("%s: xxxa30 slim_disconnect_ports ret = [%d]\n", __func__, ret);
