...
*/
	if (!wait_event_timeout(ak49xx_res->pm_wq,
		((ak49xx_pm_cmpxchg(ak49xx_res, AK49XX_PM_SLEEPABLE, AK49XX_PM_AWAKE) == AK49XX_PM_SLEEPABLE) ||
			(ak49xx_pm_cmpxchg(ak49xx_res, AK49XX_PM_SLEEPABLE, AK49XX_PM_AWAKE) == AK49XX_PM_AWAKE)),
...
static irqreturn_t ak49xx_irq_thread(int irq, void *data)
{
	...
	if (ret < 0) {
		...
		dev_dbg(ak49xx_res->dev, "wrong status: irq = %d\n", ak49xx_res->irq);
		ak49xx_unlock_sleep(ak49xx_res);
		return IRQ_NONE;
...
int ak49xx_irq_init(struct ak49xx_core_resource *ak49xx_res)
{
	...
	ret = request_threaded_irq(ak49xx_res->irq, NULL, ak49xx_irq_thread,
			IRQF_TRIGGER_LOW | IRQF_ONESHOT, "ak49xx", ak49xx_res);
