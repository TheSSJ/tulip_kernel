...
#define GF_SPIDEV_NAME	"qcom,gx_fp"
...
/* variable used to distinguish different hw */
static char *gf_version = "FP_version:";
int gf_ver = -1;
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_SETSPEED:
		...
		if (retval == 0) {
			...
			FP_LOG(ERR, "Failed to get speed from user. retval = %d\n",	retval);
		}
#else
	...
	case GF_IOC_SENDKEY:
		if (copy_from_user
			...
			FP_LOG(ERR, "Failed to copy data from user space.\n");
			retval = -EFAULT;
			break;
		}
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	FP_LOG(INFO, "entry in GF_NETLINK_ENABLE\n");
	sendnlmsg(&temp);
#elif defined(GF_FASYNC)
	struct gf_dev *gf_dev = &gf;

	FP_LOG(INFO, "entry in GF_FASYNC\n");
	if (gf_dev->async)
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
		...
		FP_LOG(INFO, "Succeed to open device. irq = %d\n",
		   gf_dev->irq);
		if (gf_dev->users == 1)
		gf_enable_irq(gf_dev);
		/*power the sensor*/
		...
		FP_LOG(ERR, "No device for minor %d\n", iminor(inode));
	}
...
static struct device_attribute gf_version_attr =
	__ATTR(gf_version, S_IRUGO, show_hw_version, NULL);
...
#endif
{
	...
	gf_dev->fp_id_gpio = -EINVAL;

	/* 1 is goodix */
	gf_ver = check_fp_id(gf_dev);
	FP_LOG(ERR, "gf_ver is %d\n", gf_ver);

	if (gf_ver == -1) {
		FP_LOG(ERR, "get hw_version failed\n");
		/* goto error; */
	} else if (gf_ver == 1) {
		FP_LOG(INFO, "begin create goodix node\n");
		gf_version = "fp_version is :goodix";
		ret = device_create_file(&pdev->dev, &gf_version_attr);
		if (ret) {
			FP_LOG(ERR, "failed: create hw_version file\n");
			gpio_free(gf_dev->fp_id_gpio);
			/* return -1; */
		}
		FP_LOG(INFO, "test is goodix\n");
		/* goto error; */
	} else {
		FP_LOG(ERR, "hw version error\n");
		gpio_free(gf_dev->fp_id_gpio);
		/* goto error; */
	}
	...
	*/
	...
	if (minor < N_SPI_MINORS) {
		...
		FP_LOG(ERR, "no minor number available!\n");
		status = -ENODEV;
	}
	...
	if (status == 0) {
		...
		if (gf_dev->input == NULL) {
			FP_LOG(ERR, "Faile to allocate input device.\n");
			status = -ENOMEM;
		}
	#ifdef AP_CONTROL_CLK
	...
	FP_LOG(INFO, "probe end successfully\n");
...
static int __init gf_init(void)
{
	...
	*/
	FP_LOG(INFO, "gf_init entry");
	BUILD_BUG_ON(N_SPI_MINORS > 256);
	status = register_chrdev(SPIDEV_MAJOR, CHRD_DRIVER_NAME, &gf_fops);
	if (status < 0) {
		FP_LOG(ERR, "Failed to register char device!\n");
		return status;
	}
	gf_class = class_create(THIS_MODULE, CLASS_NAME);
	if (IS_ERR(gf_class)) {
		unregister_chrdev(SPIDEV_MAJOR, gf_driver.driver.name);
		FP_LOG(ERR, "Failed to create class.\n");
	...
	if (status < 0) {
		...
		unregister_chrdev(SPIDEV_MAJOR, gf_driver.driver.name);
		FP_LOG(ERR, "Failed to register SPI driver.\n");
	}
