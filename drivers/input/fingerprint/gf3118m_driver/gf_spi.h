...
#define FP_LOG(debug_level_mask, fmt, ...)	\
	do {	\
		if (gf_log_mask & GF_LOG_##debug_level_mask)	\
			pr_info("<<goodix_fp-%s>>[%s:%d]:"fmt,	\
			#debug_level_mask, __func__, __LINE__, ##__VA_ARGS__);	\
	} while (0)
...
#define GF_FASYNC	/*If support fasync mechanism.*/
...
struct gf_dev {
	...
	signed pwr_gpio;
	signed fp_id_gpio;
	struct pinctrl *fp_id_pinctrl;
	struct pinctrl_state *fp_id_state;
