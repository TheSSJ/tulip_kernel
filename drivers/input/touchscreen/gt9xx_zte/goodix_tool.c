...
static void tool_set_proc_name(char *procname, int len)
{
	...
	snprintf(procname, len, "gmnode%04d%02d%02d", n_year, n_month, n_day);
}
...
s32 init_wr_node(struct i2c_client *client)
{
	...
	tool_set_proc_name(procname, sizeof(procname));
	...
	if (cmd_head.wr % 2)
		...
		while (data_len > 0) {
			...
			ret = simple_read_from_buffer(&page[loc],
				size, ppos, &cmd_head.data[GTP_ADDR_LENGTH], len);
			if (ret < 0) {
				return ret;
			}
			loc += len;
