...
/* the marcos cannt be enable,it differ from tp fw */
...
struct delayed_work gProbeWork;
struct workqueue_struct *gProbeWorkqueue;

#ifdef CONFIG_CREATE_DEBUG_INTERFACE
struct synaptics_proc_entry gProcEntry;
atomic_t inter_count = ATOMIC_INIT(0);
atomic_t report_count = ATOMIC_INIT(0);
#endif

#ifdef CONFIG_CREATE_PROC_INTERFACE
struct proc_dir_entry *mt_entry;
#endif

#ifdef CONFIG_CREATE_GLOVED_INTERFACE
#define	GLOVED_MODE_CFG_ADDR	0x0024
#define	GLOVED_MODE_CFG_ENABLE	(1<<5)
#define	GLOVED_MODE_CFG_DISABLE	~(1<<5)
#define	GLOVED_MODE_CFG_FORCE	0x03
...
static inline int synaptics_rmi4_f11_abs_wake_gesture(struct synaptics_rmi4_data
		*rmi4_data, struct synaptics_rmi4_fn *fhandler, unsigned char detected_gestures,
		unsigned int *code)
{
	int retval;
	...
	if (detected_gestures) {
		*code = KEY_RESERVED;
		/*pr_info("%s:detected_gestures is %d\n",__func__,detected_gestures);*/

		if (detected_gestures == 1) {
			pr_info("%s,GESTURE_TAP\n", __func__);
			*code = GESTURE_TAP;
		} else if (detected_gestures == 2) {

			unsigned char direction;

			retval = synaptics_rmi4_reg_read(rmi4_data,	0x0400, &direction, 1);

			if (retval < 0)
				return 0;

			pr_info("%s:swipe direction is 0x%x\n", __func__, direction);
			switch (direction) {
			case 0x8:
				pr_info("%s,GESTURE_UP\n", __func__);
				*code = GESTURE_UP;
				break;
			case 0x4:
				pr_info("%s,GESTURE_DOWN\n", __func__);
				*code = GESTURE_DOWN;
				break;
			case 0x2:
				pr_info("%s,GESTURE_LEFT\n", __func__);
				*code = GESTURE_LEFT;
				break;
			case 0x1:
				pr_info("%s,GESTURE_RIGHT\n", __func__);
				*code = GESTURE_RIGHT;
				break;
			default:
				pr_info("%s:not know this swipe direction\n", __func__);
			}
		} else if (detected_gestures == 8) {
			pr_info("%s,GESTURE_O\n", __func__);
			*code = GESTURE_O;
		} else if (detected_gestures == 32) {
			pr_info("%s,GESTURE_V\n", __func__);
			*code = GESTURE_V;
			...
			pr_info("%s,character is 0x%x\n", __func__, character[8]);
			switch (character[8])	{
			case 0x63:
				pr_info("%s,KEY_PHONE\n", __func__);
				*code = 0x229;
			...
			default:
				pr_info("%s:not know this character\n", __func__);
			}
		} else {
			pr_info("%s:not support this gestures\n", __func__);
		}
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		pr_info("%s, get a gesture,add is %x\n", __func__, data_addr + extra_data->data38_offset);
		retval = synaptics_rmi4_reg_read(rmi4_data,
		...
		if (synaptics_rmi4_f11_abs_wake_gesture(rmi4_data, fhandler, detected_gestures, &code)) {
			return 0;
		}
...
#ifdef CONFIG_CREATE_GLOVED_INTERFACE
			/*add by zte, for force glove mode.start*/
			if ((g_gloved_flag == GLOVED_MODE_CFG_FORCE) && (y > FORCE_GLOVED_MODE_MAX_Y) &&
			    (g_hall_flag == true)) {
...
#ifdef REPORT_2D_W
			input_report_abs(rmi4_data->input_dev,
					 ABS_MT_TOUCH_MAJOR, max_t(int, wx, wy));
			input_report_abs(rmi4_data->input_dev,
					 ABS_MT_TOUCH_MINOR, min_t(int, wx, wy));
#endif
...
static inline int synaptics_rmi4_f12_abs_wake_gesture(struct synaptics_rmi4_data
		*rmi4_data, struct synaptics_rmi4_fn *fhandler, unsigned char detected_gestures,
		unsigned int *code)
{
	...
	if (detected_gestures == 0x0) {
		pr_info("%s,GESTURE_TAP\n", __func__);
		*code = GESTURE_TAP;
	} else if (detected_gestures == 0x07) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
						 0x041A, &TempData, sizeof(TempData));

		if (retval < 0) {
			pr_info("%s:read gesture direction failed!!!\n", __func__);
			return -EINVAL;
		}

		pr_info("%s:direction is 0x%x\n", __func__, TempData);
		switch (TempData) {
		case 0x48:
			pr_info("%s,GESTURE_UP\n", __func__);
			*code = GESTURE_UP;
		...
		case 0x41:
			...
			*code = GESTURE_RIGHT;
			break;
		default:
			pr_info("%s:unknown this direction\n", __func__);
			return -EINVAL;
		}
	} else if (detected_gestures == 0x08) {
		pr_info("%s,gestures O\n", __func__);
		*code = GESTURE_O;
	} else if (detected_gestures == 0x0A) {
		pr_info("%s,gestures V\n", __func__);
		*code = GESTURE_V;
	} else {
		pr_info("%s,unknown this gestures\n", __func__);
		return -EINVAL;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (retval < 0)
			return 0;
		/*
				if(synaptics_rmi4_f12_abs_wake_gesture(rmi4_data,fhandler, gesture_type, &code))
				{
					return 0;
				}
		*/
		if (rmi4_data->gesture_detection[0] == 0x03) {
			pr_info("%s,GESTURE_TAP\n", __func__);
			input_report_key(rmi4_data->input_dev, GESTURE_TAP, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, GESTURE_TAP, 0);
			input_sync(rmi4_data->input_dev);
			/*rmi4_data->suspend = false;*/
		}
...
#ifdef CONFIG_CREATE_GLOVED_INTERFACE
		/*add by zte, for force glove mode.start*/
		if ((g_gloved_flag == GLOVED_MODE_CFG_FORCE)  && (g_hall_flag == true))
			continue;
		/*add by zte, end*/
#endif
		...
		bool report)
{
	...
	if (rmi4_data->update_flag == true)
		pr_info("%s: found interrupt in update firmware!!!!, intr %02X\n", __func__,  intr[0]);
	...
	if (status.unconfigured && !status.flash_prog) {
		...
		if (rmi4_data->update_flag == false) {
				     ...
				     bool enable, bool attn_only)
{
	...
	pr_info("%s: enable: %d, attn_only: %d\n", __func__, enable, attn_only);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!bdata->cap_button_map) {
		...
		if (bdata->cap_button_map->nbuttons != f1a->max_count) {
			f1a->valid_button_count = min_t(unsigned char, f1a->max_count,
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		set_bit(GESTURE_TAP, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_TAP);

		set_bit(GESTURE_V, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_V);

		set_bit(GESTURE_O, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_O);

		set_bit(GESTURE_UP, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_UP);

		set_bit(GESTURE_DOWN, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_DOWN);

		set_bit(GESTURE_LEFT, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_LEFT);

		set_bit(GESTURE_RIGHT, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_RIGHT);

		set_bit(GESTURE_CHAR_W, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, GESTURE_CHAR_W);
	}
				     ...
				     bool enable)
{
	...
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	if (g_enable == enable) {
		pr_info("%s: same enable reg\n", __func__);
		goto exit;
	} else {
		/*pr_info("%s: different enable reg\n",__func__);*/
		g_enable = enable;
	}
...
exit:
	...
	/*add by zte, update firmware info,start*/
	synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f34_ctrl_base_addr,
				rmi4_data->firmware_information, 4);
	pr_info("synaptics firmware_information: 0x%x, 0x%x, 0x%x, 0x%x\n",
	...
	/*add by zte, update firmware info, end*/
...
#if (defined CONFIG_CREATE_DEBUG_INTERFACE) || (defined CONFIG_CREATE_PROC_INTERFACE)
static int synaptics_firmware_versions_parse(struct seq_file *m,
		unsigned char *firmware_id)
{

	switch (firmware_id[1]) {
	...
	case 0x32:
		...
		break;
	case 0x33:
		seq_printf(m, "%s-", "Success");
		break;
	case 0x34:
		seq_printf(m, "%s-", "Oflim");
		break;
	case 0x35:
		seq_printf(m, "%s-", "Lead");
		break;
	case 0x36:
		seq_printf(m, "%s-", "Wintek");
		break;
	case 0x37:
		seq_printf(m, "%s-", "Laibao");
		break;
	case 0x38:
		seq_printf(m, "%s-", "CMI");
		break;
	case 0x39:
		...
		break;
	case 0x41:
		seq_printf(m, "%s-", "Goworld");
		break;
	case 0x42:
		seq_printf(m, "%s-", "Baoming");
		break;
	case 0x43:
		seq_printf(m, "%s-", "Eachopto");
		break;
	case 0x44:
		seq_printf(m, "%s-", "Mutto");
		break;
	case 0x45:
		seq_printf(m, "%s-", "Junda");
		break;
	case 0x46:
		seq_printf(m, "%s-", "Jdi");
		break;
	...
	seq_printf(m, "%02X%02X-", firmware_id[2], firmware_id[3]);
...
#ifdef CONFIG_CREATE_DEBUG_INTERFACE
/*add by zte 20150309,start*/

static int tp_reg_read_f01_ctrl_info(struct seq_file *m)
{
	unsigned char ReadData;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;

	synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_ctrl_base_addr, &ReadData,
				sizeof(ReadData));

	/*------------------------------------------------------------------*/
	seq_printf(m, "%s", "\n\n>>>>> F01 CTRL REG <<<<<\n\n");

	seq_printf(m, "%s", "SleepMode:		");
	switch (ReadData & MASK_2BIT) {
	case 0x00:
		seq_printf(m, "%s\n", "Normal Operation");
		break;
	case 0x01:
		...
		break;
	case 0x02:
		seq_printf(m, "%s\n", "Sensor Sleep Without Recalibration");
		break;
	case 0x03:
		seq_printf(m, "%s\n", "Reserved");
		break;
	...
	seq_puts(m, "NoSleep:		%s\n", (ReadData & 0x04) ? "yes" : "no");

	seq_puts(m, "ChargerConnected:	%s\n", (ReadData & 0x20) ? "yes" : "no");

	seq_puts(m, "Configured:		%s\n", (ReadData & 0x80) ? "yes" : "no");

	synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_ctrl_base_addr + 1, &ReadData,
				sizeof(ReadData));

	seq_printf(m, "IntEnable:		0x%02X\n", ReadData);

	return 0;
}

static int tp_reg_read_f01_data_info(struct seq_file *m)
{
	unsigned char ReadData;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;

	synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_data_base_addr, &ReadData,
				sizeof(ReadData));

	/*------------------------------------------------------------------*/
	seq_printf(m, "%s", "\n\n>>>>> F01 DATA REG <<<<<\n\n");

	seq_printf(m, "%s", "StatusCode:		");
	switch (ReadData & MASK_3BIT) {
	case 0x00:
		seq_printf(m, "%s\n", "No Error.");
		break;
	case 0x01:
	...
	seq_puts(m, "FlashProg:		%s\n", (ReadData & 0x40) ? "yes" : "no");

	seq_puts(m, "Unconfigured:		%s\n", (ReadData & 0x80) ? "yes" : "no");

	return 0;
}

static int tp_reg_read_f12_ctrl_info(struct seq_file *m)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
	...
	seq_printf(m, "%s", "ReportingMode:		");
	switch (reporting_control[2] & MASK_2BIT) {
	case 0x00:
		seq_printf(m, "%s\n", "Continuous, when finger present");
		break;
	case 0x01:
		seq_printf(m, "%s\n", "Reporting Always");
		break;
	case 0x02:
		seq_printf(m, "%s\n", "Wakeup gesture reporting mode");
		break;
	default:
		seq_printf(m, "%s\n", "Unnormal");
	}
...
static int tp_reg_read_status_info(struct seq_file *m)
{
	...
	seq_puts(m, "irq enabled:		%s\n", rmi4_data->irq_enabled ? "yes" : "no");

	seq_printf(m, "irq count:		%u\n", atomic_read(&inter_count));

	seq_printf(m, "report count:		%u\n", atomic_read(&report_count));

	seq_printf(m, "wakeup gesture enable:	%s\n",
		   rmi4_data->enable_wakeup_gesture ? "yes" : "no");

	return 0;
}

static int tp_reg_read(struct seq_file *m, void *v)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;

	tp_reg_read_status_info(m);

	if (rmi4_data->suspend && !rmi4_data->enable_wakeup_gesture)
		return 0;

	tp_reg_read_f01_ctrl_info(m);

	tp_reg_read_f01_data_info(m);

	tp_reg_read_f12_ctrl_info(m);
...
{
	return single_open(file, tp_reg_read, NULL);
}
...
static int FirmwareVer_read(struct seq_file *m, void *v)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;

	synaptics_firmware_versions_parse(m, rmi4_data->firmware_information);

	return 0;
}


static int FirmwareVer_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, FirmwareVer_read, NULL);
}

static const struct file_operations FirmwareVer_id = {
	.open		= FirmwareVer_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
{
	memset(&gProcEntry, 0, sizeof(struct synaptics_proc_entry));

	gProcEntry.proc_path_entry = proc_mkdir("driver/TP_DEBUG", NULL);
	if (gProcEntry.proc_path_entry) {
		pr_info("create TP_DEBUG success\n");
	} else {
		pr_info("create TP_DEBUG fail\n");
		return;
	}

	gProcEntry.debug_entry = proc_create("driver/TP_DEBUG/SynRegDebug", 0444, NULL,
					     &debug_id);
	if (gProcEntry.debug_entry) {
		pr_info("create SynRegDebug success\n");
	} else {
		pr_info("create SynRegDebug fail\n");
		return;
	}

	gProcEntry.firmware_ver_entry = proc_create("driver/TP_DEBUG/FirmwareVer", 0444, NULL,
					&FirmwareVer_id);
...
static void synaptics_zte_debug_work(void)
{
	...
	pr_info("[TP] *************** IntEnable:0x%02X\n", ReadData);

	synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_data_base_addr, &ReadData,
				sizeof(ReadData));

	/*------------------------------------------------------------------*/
	pr_info("[TP] %s", "StatusCode:");
	switch (ReadData & MASK_3BIT) {
	case 0x00:
		pr_info("%s\n", "No Error.");
		break;
	...
	if (gpio_get_value(bdata->irq_gpio) == bdata->irq_on_state)
		pr_info("[TP] [ERROR] XXXXX In resume: irq_gpio == irq_on_state XXXXX\n");
...
#ifdef CONFIG_CREATE_PROC_INTERFACE
static int mt_tpd_read(struct seq_file *m, void *v)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
...
{
	return single_open(file, mt_tpd_read, NULL);
}
...
void write_gloved_value(u8 value)
{
	...
	pr_info("synaptics [%s] write gloved mode [%u] !!!\n", __func__, value);
	...
	retval = synaptics_rmi4_reg_read(rmi4_data, fhandler->full_addr.ctrl_base + offset,
					 &ctrl_23, 1);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	pr_info("=== syna [begin]%s ,line=%d,current_gloved_finger_state=0x%x, ctrl_23 0x%x, value = %d.\n",
		__func__, __LINE__, current_gloved_finger_state, ctrl_23, value);
	...
	pr_info("=== syna [end]%s ,line=%d,current_gloved_finger_state=0x%x, ctrl_23 0x%x.\n",
		__func__, __LINE__, current_gloved_finger_state, ctrl_23);
...
int gloved_finger_hall_status(bool flag)
{
	...
	pr_info("synaptics [%s] [%d] update hall flag %u\n", __func__, __LINE__, flag);
...
/*EXPORT_SYMBOL(gloved_finger_hall_leave);*/
...
static int gloved_finger_proc_show(struct seq_file *m, void *v)
{
	...
	if (rmi4_data->suspend == true)
		...
		seq_printf(m,
			   "current_gloved_finger_state:%d, g_gloved_flag %d, register ctrl_23 0x%x.\n",
					...
					size_t count, loff_t *pos)
{
	...
	if (input <= 2)
		...
		pr_info("XXXXX synaptics set gloved mode value error!!!! XXXXX");
...
static int gloved_finger_proc_init(void)
{
	...
	if (!res) {
		pr_info("synaptics failed to create /proc/gloved_finger_switch\n");
		return -ENOMEM;
	}

	pr_info("synaptics created /proc/gloved_finger_switch\n");
	return 0;
}
#else
...
#ifdef CONFIG_WAKE_GESTURE_INTERFACE
unsigned char gwake_gesture = 0;

static ssize_t wake_gesture_proc_write(struct file *file, const char __user *buffer,
				       size_t count, loff_t *pos)
{
	unsigned int input = 0;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
...
static int wake_gesture_proc_show(struct seq_file *m, void *v)
{
	/*unsigned char gloved_mode = 0;*/
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
...
static int wake_gesture_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, wake_gesture_proc_show, NULL);
}

static const struct file_operations wake_gesture_proc_fops = {
	.open		= wake_gesture_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release		= single_release,
	.write		= wake_gesture_proc_write,
};

static int wake_gesture_proc_init(void)
{
	...
	if (!res) {
		pr_info("synaptics failed to create /proc/touchscreen dir\n");
		return -ENOMEM;
	}

	res = proc_create("touchscreen/wake_gesture", S_IWUGO | S_IRUGO, NULL,
			  &wake_gesture_proc_fops);
	if (!res) {
		pr_info("synaptics failed to create /proc/touchscreen/wake_gesture_switch\n");
		return -ENOMEM;
	}

	pr_info("synaptics created /proc/touchscreen/wake_gesture_switch\n");
	return 0;
}
...
{
	if (strnstr(str, "ffbm-01", strlen(str))) {
		g_boot_ffbm = 1;
	}
	return 1;
}
...
static void synaptics_rmi4_probe_work(struct work_struct *work)
{
	int retval;
	unsigned char attr_count;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
	...
	const struct synaptics_dsx_board_data *bdata;
	struct platform_device *pdev = rmi4_data->pdev;
	...
	/* Note:Pinctrl must be set or fwu occurs error */
	retval = synaptics_rmi4_pinctrl_init(rmi4_data);
	if (!retval && rmi4_data->ts_pinctrl) {
		retval = synpatics_rmi4_pinctrl_select(rmi4_data, true);
		if (retval < 0)
			pr_info("pzh:synaptics_rmi4_probe---Failed to select pin to active state");
	}
	...
	INIT_DELAYED_WORK(&rmi4_data->resume_work,
			  synaptics_rmi4_resume_work);/*add by zte,improve open speed of lcd*/
	INIT_DELAYED_WORK(&rmi4_data->suspend_work,
			  synaptics_rmi4_suspend_work);/*add by zte,improve open speed of lcd*/
...
#ifdef CONFIG_CREATE_PROC_INTERFACE
	mt_entry = proc_create("touchscreen/ts_information", 0444, NULL, &tsc_id);
	if (mt_entry) {
		pr_info("create tsc_id success\n");
	} else {
		pr_info("create tsc_id fail\n");
	}
#endif
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	memset(rmi4_data, 0, sizeof(*rmi4_data));
	...
	*/
	...
	exp_data.rmi4_data = rmi4_data;/*add by zte*/

	gProbeWorkqueue = create_singlethread_workqueue("zte_tp_probe_workqueue");

	INIT_DELAYED_WORK(&gProbeWork, synaptics_rmi4_probe_work);
				  ...
				  bool enable)
{
	...
	pr_info("%s,ctrl_base is %x, %x\n", __func__, fhandler->full_addr.ctrl_base, offset);
	retval = synaptics_rmi4_reg_write(rmi4_data,
					  fhandler->full_addr.ctrl_base + offset,
					  reporting_control,
					  sizeof(reporting_control));
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	queue_delayed_work(rmi4_data->rb_workqueue,
			   &rmi4_data->suspend_work,
			   msecs_to_jiffies(0));

	suspend_flag = 0;

	ret = wait_event_interruptible_timeout(suspend_waitq, suspend_flag, HZ);
...
static void synaptics_rmi4_suspend_work(struct work_struct *work)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;/*add by cc10172188*/
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	/*struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);*/
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	...
	if (rmi4_data->stay_awake || rmi4_data->suspend == true) {
		suspend_flag = 1;
		wake_up_interruptible(&suspend_waitq);
		return;
	}

	/*add by zte, en...may be is bug when press the power key many times, start*/
	/*cancel_delayed_work_sync(&rmi4_data->resume_work);*/
	/*add by zte ,end*/

	rmi4_data->enable_wakeup_gesture =
		gwake_gesture;/* add by zte, it's bug when set wakeup gesture in tp suspend*/

	if (rmi4_data->enable_wakeup_gesture) {
		pr_info("%s,write gesture\n", __func__);
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		enable_irq_wake(rmi4_data->irq);
		goto exit;
	}
...
#ifdef CONFIG_CREATE_DEBUG_INTERFACE
	pr_info("[TP] *************** In suspend: irq count %u, report count %u\n",
...
static void synaptics_rmi4_resume_work(struct work_struct *work)
{
	...
	retval = synaptics_rmi4_enable_reg(rmi4_data, true);
	if (retval < 0) {
		pr_info("%s: synaptics_rmi4_enable_reg failed\n", __func__);
	}
