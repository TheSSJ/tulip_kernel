...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (of_find_property(np, "synaptics,swap-axes", NULL))
		bdata->swap_axes = of_property_read_bool(np, "synaptics,swap-axes");
	else
		bdata->swap_axes = false;

	if (of_find_property(np, "synaptics,x-flip", NULL))
		bdata->x_flip = of_property_read_bool(np, "synaptics,x-flip");
	else
		bdata->x_flip = false;

	if (of_find_property(np, "synaptics,y-flip", NULL))
				    ...
				    const struct i2c_device_id *dev_id)
{
	...
	pr_info("pzh:Enter synaptics_rmi4_i2c_probe\n");
