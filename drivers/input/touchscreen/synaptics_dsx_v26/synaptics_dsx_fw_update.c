...
#define FW_IMAGE_NAME_TPK       "TPK-PR2030544-S7020_36314945.img"
#define FW_IMAGE_NAME_TRULY     "Truly-PR2030544-S7020_36323942.img"
#define FW_IMAGE_NAME_SUCCESS   "Success-PR1953015-S7020_36333939.img"
#define FW_IMAGE_NAME_SAMSUNG   "Samsung-PR2312302-S3718_common.img"
...
static int fwu_get_image_firmware_id(unsigned int *fw_id)
{
	...
	if (fwu->img.contains_firmware_id) {
		pr_info("%s,contains firmware\n", __func__);
		*fw_id = fwu->img.firmware_id;
	} else {
		pr_info("%s,read firmware\n", __func__);
...
static int fwu_start_reflash(void)
{
	...
	if (g_boot_ffbm == 1) {
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: no update firmware when PV mode\n",
			__func__);
	}
	...
	if (fwu->image == NULL) {
#if defined(CONFIG_BOARD_TULIP)
		memset(fwu->image_name, 0, MAX_IMAGE_NAME_LEN);
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				SYN_LCE_FW_NAME, strlen(SYN_LCE_FW_NAME),
				strlen(SYN_LCE_FW_NAME));
	...
	case UI_CONFIG:
		retval = fwu_check_ui_configuration_size();
		if (retval < 0) {
			pr_info("%s,check failed\n", __func__);
			break;
		}
		fwu->config_area = UI_CONFIG_AREA;
		retval = fwu_erase_configuration();
		if (retval < 0) {
			pr_info("%s,erase failed\n", __func__);
			break;
		}
		retval = fwu_write_ui_configuration();
		rmi4_data->reset_device(rmi4_data, true);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	fwu->productinfo = rmi4_data->rmi4_mod_info.product_info[0];
