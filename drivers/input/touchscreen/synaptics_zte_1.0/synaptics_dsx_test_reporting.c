...
\
	for (ii = 0; ii < length; ii++) {\
		retval = kstrtouint(buf, 0, &setting);\
		if (!retval)\
...
static void synaptics_rmi4_f54_sensor_mapping(void)
{
	...
	pr_info("RxTx mapped from F$%s\t, Rx mapped count %d(%d)\t, Tx mapped count %d(%d)\n",
			f54->fn55 ? "55" : "54",
			rmi4_data->num_of_rx,
			rx_len,
			rmi4_data->num_of_tx,
...
static void synaptics_rmi4_f54_status_work(struct work_struct *work)
{
	...
	pr_info("%s:start to get report data!\n", __func__);
	if (f54->status != STATUS_BUSY)
		return;
	...
	pr_info("%s: read report data successfully!\n", __func__);
