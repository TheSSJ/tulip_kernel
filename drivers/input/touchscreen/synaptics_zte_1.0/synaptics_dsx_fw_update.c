...
static enum flash_area fwu_go_nogo(struct image_header *header)
{
	...
	if (header->is_contain_build_info) {
		...
		strptr = strnstr(fwu->firmware_name, "PR", strlen(fwu->firmware_name));
		if (!strptr) {
			dev_err(&i2c_client->dev,
				"%s: No valid PR number found in image file name\n",
				__func__);
			goto exit;
		}
	...
	pr_info("%s: Device firmware id %d, .img firmware id %d\n",
		__func__,
		deviceFirmwareID,
	...
	pr_info("%s: Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
		__func__,
		config_id[0], config_id[1], config_id[2], config_id[3]);

	/* .img config id */
	pr_info("%s .img config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
		__func__,
		fwu->config_data[0],
		fwu->config_data[1],
		fwu->config_data[2],
	...
	pr_info("%s: Device config ID %d, .img config ID %d\n",
...
int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source)
		...
		if ((sensor_pin1 == 0) && (sensor_pin2 == 0))
			strlcat(fwu->firmware_name, SENSOR_VENDOR_1, sizeof(fwu->firmware_name));
		else if ((sensor_pin1 == 0) && (sensor_pin2 == 1))
			strlcat(fwu->firmware_name, SENSOR_VENDOR_2, sizeof(fwu->firmware_name));
		else if ((sensor_pin1 == 1) && (sensor_pin2 == 0))
			strlcat(fwu->firmware_name, SENSOR_VENDOR_3, sizeof(fwu->firmware_name));
		else
			dev_err(&fwu->rmi4_data->i2c_client->dev,
		...
		strlcat(fwu->firmware_name, ".img", sizeof(fwu->firmware_name));
#else
		pr_info("syna fwfile name:%s\n", syna_file_name);

		if (!syna_file_name) {
			pr_info("syna fw name is null\n");
			return -EINVAL;
		}
		if (!strcmp(syna_file_name, "")) {
			pr_info("%s file_name is null\n", __func__);
			return -EINVAL;
		}
...
int syna_get_fw_ver(struct i2c_client *client, char *pfwfilename)
{
	...
	if (!strcmp(pfwfilename, charcmp)) {
		pr_info("syna_get_fw_ver pfwfilename is null\n");
		return -EINVAL;
	}
	pr_info("syna_get_fw_ver fw name:%s\n", pfwfilename);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	pr_info("synaptics_rmi4_fwu_init\n");

	if (rmi4_data == NULL) {
		pr_info("%s:rmi4_data is null\n", __func__);
		return -EINVAL;
	}
	...
	fwu->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue");
		pr_info("%s: build fw update work queue\n", __func__);
...
{
	pr_info("rmi4_fw_update_module_init\n");
