		      ...
		      const char *text, u16 text_len)
{
	...
	msg->pid = current->pid;
	snprintf(msg->comm, sizeof(msg->comm) - 1, "%s", current->comm);
	/*zte add end*/
...

{
	...
	/* zte_pm_20151126. copy tongchangda 8952-L, sync kernel&system log-time (twist kernel timestamp) */
#if 0
	if (!buf)
	...
	info_len = sprintf(buf + tlen,
	"[%u][%d: %s]", process_id, pid, comm);
...
static bool cont_add(int facility, int level, const char *text, size_t len)
{
	...
	if (!cont.len) {
		...
		snprintf(cont.comm, sizeof(cont.comm - 1),
		"%s", current->comm);
		/* zte add */
	}
